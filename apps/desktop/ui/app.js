...
<div style="min-height:100vh;background:#0a0605;color:#f7eee7;display:flex;flex-direction:column;font-size:13px;font-variant-numeric:tabular-nums;">
  ...
  const panel = state.nav==='perfis' ? renderProfilesPanel() : state.nav==='rede' ? renderNetworkPanel() : state.nav==='mapas' ? renderCollectorPanel() : state.nav==='logs' ? renderLogsPanel() : '';
  document.getElementById('root').innerHTML = html + panel + (state.mapPicker ? renderMapPicker() : '') + (state.protonSetupAccount ? renderProtonSetup() : '');
...
function renderNetworkPanel(){const cards=state.live.map(account=>{const network=account.network||{},result=network.result||{},profile=state.networkProfiles.find(item=>item.id===(network.profileId||account.networkProfileId));const color=result.status==='OK'?'#7fd9a2':result.status?'#e48275':'#8a7a70';return`<section style="border:1px solid #39231b;border-radius:12px;padding:16px;background:#120c0a;"><div style="display:flex;justify-content:space-between"><b>${esc(account.name)}</b><span style="color:${color};font-weight:700">${esc(result.status||'NÃƒO TESTADO')}</span></div><div style="margin:12px 0;color:#b5a196;line-height:1.7">Perfil: <b>${esc(profile?.name||network.profileName||'â€”')}</b><br>Provider: ${esc(network.provider||profile?.type||'â€”')}<br>IP atual: ${esc(result.publicIp||'â€”')}<br>PaÃ­s: ${esc(result.country||'â€”')} Â· LatÃªncia: ${result.latencyMs!=null?`${result.latencyMs} ms`:'â€”'}<br>Ãšltimo teste: ${esc(result.checkedAt||'â€”')}${result.error?`<br><span style="color:#e48275">${esc(result.error)}</span>`:''}${account.protonSetup?.state?`<br>Proton: <b>${esc(account.protonSetup.state)}</b>`:''}</div><select data-network-select="${esc(account.id)}" style="width:100%;height:34px;background:#090504;color:#f7eee7;border:1px solid #493025;border-radius:7px">${state.networkProfiles.map(p=>`<option value="${esc(p.id)}" ${p.id===account.networkProfileId?'selected':''}>${esc(p.name)} Â· ${esc(p.type)}</option>`).join('')}</select><div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap"><button data-network-test="${esc(account.id)}" class="vp-primary">Testar</button><button data-network-change="${esc(account.id)}" class="vp-ghost">Trocar perfil</button><button data-proton-setup="${esc(account.id)}" class="vp-ghost">Configurar Proton</button>${network.reconnectSupported?`<button data-network-reconnect="${esc(account.id)}" class="vp-ghost">Reconectar</button>`:''}</div></section>`}).join('');const form=`<form data-network-profile-form style="margin-top:18px;padding:16px;border:1px solid #39231b;border-radius:12px;display:grid;grid-template-columns:1.2fr .7fr .7fr 1.2fr .6fr .7fr .8fr auto;gap:8px"><input name="name" placeholder="Nome do perfil" required><select name="protocol"><option>http</option><option>https</option><option>socks4</option><option>socks5</option></select><input name="country" placeholder="PaÃ­s (BR)"><input name="host" placeholder="Host do proxy" required><input name="port" type="number" placeholder="Porta" required><input name="username" placeholder="UsuÃ¡rio"><input name="password" type="password" placeholder="Senha"><button class="vp-primary">Salvar proxy</button></form><div style="margin-top:8px;color:#7d6d64;font-size:11px">Proxy autenticado Ã© armazenado no Vault, mas permanece bloqueado atÃ© haver preflight autenticado confiÃ¡vel; nÃ£o existe fallback para Direct.</div>`;return panelShell('Rede por conta','Preflight aplicado na Session isolada antes de carregar o jogo.',`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px">${cards}</div>${form}`);}
function renderProtonSetup(){const account=state.live.find(item=>item.id===state.protonSetupAccount);return`<div style="position:fixed;z-index:60;inset:18px;background:#0b0706;border:1px solid #6a422c;border-radius:14px;overflow:hidden;display:flex;flex-direction:column"><div style="height:52px;display:flex;align-items:center;padding:0 16px;gap:12px;border-bottom:1px solid #39231b"><b style="color:#e5b34f">Proton individual Â· ${esc(account?.name||'')}</b><span style="color:#8a7a70">Login oficial isolado nesta conta. Gere e baixe a configuraÃ§Ã£o WireGuard.</span><button data-proton-close style="margin-left:auto">Fechar</button></div><div data-proton-host style="flex:1;min-height:0"></div></div>`;}
...
async function syncEmbeddedViews(){
  ...
  const protonHost=document.querySelector('[data-proton-host]'),rect=protonHost?.getBoundingClientRect();await window.vpNative.layoutProtonSetup?.({id:state.protonSetupAccount,visible:Boolean(protonHost&&rect),bounds:rect?{x:rect.x,y:rect.y,width:rect.width,height:rect.height}:null}).catch(()=>{});
}
...
document.addEventListener('click', async e => {
  ...
  if (el.dataset.nav != null) { state.nav = el.dataset.nav;render();if(['perfis','rede'].includes(state.nav))await loadProfiles();if(state.nav==='mapas')await loadCollector();if(state.nav==='logs')await loadLogs();return; }
...
document.addEventListener('submit',async e=>{
  const networkForm=e.target.closest('[data-network-profile-form]');if(networkForm){e.preventDefault();const f=networkForm.elements,id=`network:proxy:${Date.now()}`,result=await window.vpNative.saveNetworkProfile({id,name:f.name.value,type:'PROXY',username:f.username.value,password:f.password.value,enabled:true,config:{protocol:f.protocol.value,host:f.host.value,port:Number(f.port.value),expectedCountry:f.country.value.trim().toUpperCase()||undefined}});notice(result.ok?'Perfil de proxy salvo.':result.error);if(result.ok)await loadProfiles();return;}const form=e.target.closest('[data-profile-form]');if(!form)return;e.preventDefault();await saveProfileForm(form);
});
document.addEventListener('click',async e=>{
  const proton=e.target.closest('[data-proton-setup]');if(proton){const result=await window.vpNative.openProtonSetup(proton.dataset.protonSetup);if(!result.ok)return notice(result.error);state.protonSetupAccount=proton.dataset.protonSetup;render();return;}
  if(e.target.closest('[data-proton-close]')){const id=state.protonSetupAccount;state.protonSetupAccount=null;await window.vpNative.closeProtonSetup(id);render();return;}
  const networkTest=e.target.closest('[data-network-test]');if(networkTest){const result=await window.vpNative.testNetwork(networkTest.dataset.networkTest);notice(result.ok?`Rede OK: ${result.network.result.publicIp||'IP consultado'}`:(result.network?.result?.error||'Falha no teste de rede'));await refresh();return;}
  const networkReconnect=e.target.closest('[data-network-reconnect]');if(networkReconnect){const result=await window.vpNative.reconnectNetwork(networkReconnect.dataset.networkReconnect);notice(result.ok?'Rede reconectada e validada.':result.error);await refresh();return;}
  const networkChange=e.target.closest('[data-network-change]');if(networkChange){const id=networkChange.dataset.networkChange,profileId=document.querySelector(`[data-network-select="${id}"]`)?.value,result=await window.vpNative.changeNetworkProfile({id,profileId});notice(result.ok?'Perfil aplicado e validado.':(result.error||'Perfil bloqueado pelo preflight.'));await loadProfiles();await refresh();return;}
