...
try {
  if (window === window.top && /^(?:[^.]+\.)?pokewg\.com$/i.test(location.hostname)) {
    ...
    const readUi = (root = document) => { const controls = [...root.querySelectorAll?.("button,[role=button],a") || []].filter(visible).map(element => clean(element.innerText || element.textContent || element.getAttribute("aria-label"), 160)).filter(Boolean),text=clean(root.body?.innerText||root.documentElement?.innerText||"",20000),inputs=[...root.querySelectorAll?.("input")||[]].filter(visible); return { controls:controls.slice(0,120), maps:controls.filter(text => /\bNv\s*\d+/i.test(text)).slice(0,80), mapModalOpen:controls.some(text => /^Mapa$/i.test(text)) && controls.some(text => /\bNv\s*\d+/i.test(text)), marketOpen:controls.some(text => /Loja do Mark|Comprar Agora|Vender selecionados/i.test(text)), inventoryOpen:controls.some(text => /Mochila|Inventário|Capacidade/i.test(text)),auth:{ loginRequired:inputs.some(input=>input.type==="password")&&inputs.some(input=>/email|user|login|usu[aá]rio/i.test([input.type,input.name,input.id,input.placeholder,input.autocomplete].join(" "))),challenge:/captcha|cloudflare|verifique que você|código de verificação|autenticação em dois fatores|2fa/i.test(text) } }; };
