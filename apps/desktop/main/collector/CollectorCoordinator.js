...
export class CollectorCoordinator {
  ...
  recordUiAction(accountId, label) { const surface = this.surface(label) || "CONTROL"; const context = this.contexts.get(accountId) || {}; this.contexts.set(accountId, { ...context, lastUiAction: surface, uiActionLabel: String(label).slice(0,160), uiActionAt: Date.now() }); this.events.add({ accountId, sessionRunId: this.sessionRunIds.get(accountId), type: "UI_ACTION", payload: { surface, label: String(label).slice(0,160) } }); }
}
